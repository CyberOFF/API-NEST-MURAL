...
@Injectable()
export class MuralServices {//Inicia criando um repo, para as funçoes http
  constructor(
  ...
  async remove (name,message){
    const mural = await this.muralRepository.delete({name,message})//Deletar
    return mural//Retornar os dados
  }
